...
export default function KitWorkspacePage() {
  ...
  }, [activeTab, practiceIndex, isAnswerRevealed, kit?.flashcards]);

  // Export clean Appendix A JSON
  const handleExport = () => {
    window.open(`http://localhost:5000/api/kits/${kitId}/export`, '_blank');
  };
  ...
  const handleSaveBrief = async () => {
    ...
    try {
      ...
      const res = await fetchApi<{ kit: InternalKit }>(`/kits/${kitId}`, {
        method: 'PUT',
        body: JSON.stringify({ company_brief: updatedBrief }),
      });
      setKit(res.kit);
  ...
  const handleRegenerateBrief = async () => {
    if (!confirm('Regenerate Company Brief? This will fetch fresh insights while preserving questions and schedule.')) return;
    try {
  ...
  const handleTogglePin = async (qId: string) => {
    ...
    try {
      ...
      alert(err.message || 'Failed to pin question');
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#090A0F] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xs text-slate-400">Loading interview kit workspace...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !kit) {
    return (
      <div className="min-h-screen bg-[#090A0F] flex flex-col items-center justify-center p-6 text-center">
        <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/30 text-xs text-red-400 max-w-md">
          {error || 'Kit not found'}
        </div>
        <Link
          href="/dashboard"
          className="mt-4 inline-flex items-center space-x-2 text-xs text-indigo-400 hover:text-indigo-300"
        >
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#090A0F]">
      ...
      {/* Page 5: Workspace Header */}
      <header className="border-b border-[#1F2130] bg-[#0E1017]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center space-x-3">
            <Link
              href="/dashboard"
              className="p-1.5 rounded-md hover:bg-[#1C1E2B] text-slate-400 hover:text-slate-200 transition-colors"
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 text-[11px] text-slate-400 mt-0.5">
                <span className="inline-flex items-center px-1.5 py-0.2 rounded text-[10px] font-mono bg-[#1B1D2A] text-indigo-300 border border-[#27293C]">
          ...
          <div className="flex items-center space-x-2.5">
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1.5 rounded-md border border-[#262838] hover:bg-[#1C1E2B] text-slate-300 hover:text-white text-xs font-medium transition-colors shadow-sm"
            ...
            <button
              ...
              className="p-1.5 rounded-md border border-[#262838] hover:border-red-500/50 hover:bg-red-500/10 text-slate-400 hover:text-red-400 transition-colors"
              title="Delete Kit"
            >
        ...
        {/* Tab Navigation */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex space-x-1 overflow-x-auto border-t border-[#1B1D2A] pt-1">
          {[
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        {activeTab === 'brief' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-[#212330]">
              <div>
                ...
                {isEditingBrief ? (
                  <>
                    <button
                      ...
                      className="flex items-center space-x-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-md text-xs font-medium transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 border border-[#262838] text-slate-400 hover:text-slate-200 rounded-md text-xs font-medium"
                    >
                      Cancel
                    </button>
                  </>
                  ...
                  <>
                    <button
                      ...
                      className="flex items-center space-x-1.5 px-3 py-1.5 border border-[#262838] hover:bg-[#1C1E2B] text-slate-300 hover:text-white rounded-md text-xs font-medium transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center space-x-1.5 px-3 py-1.5 bg-[#171926] hover:bg-[#1E2133] border border-[#262838] text-slate-300 hover:text-white rounded-md text-xs font-medium transition-colors"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-8 space-y-6">
                {/* What they do */}
                <div className="bg-[#10121B] border border-[#212330] rounded-xl p-6 shadow-sm">
                  <h3 className="text-xs font-mono uppercase tracking-wider text-indigo-400 font-semibold mb-2">
                    What They Do
                  </h3>
                  {isEditingBrief ? (
                    <textarea
                      ...
                      className="w-full bg-[#0A0B10] border border-[#262838] rounded-md p-3 text-xs sm:text-sm text-slate-100 outline-none focus:border-indigo-500"
                    />
                  ) : (
              ...
              <div className="lg:col-span-4 bg-[#10121B] border border-[#212330] rounded-xl p-6 shadow-sm">
                ...
                <div className="space-y-2.5">
                  {kit.source.pages_used && kit.source.pages_used.length > 0 ? (
                    kit.source.pages_used.map((url, idx) => (
                      <div
                        key={idx}
                        className="p-2.5 rounded-lg bg-[#0C0E15] border border-[#1E202E] text-xs flex items-center justify-between"
                      >
                        <span className="text-slate-300 font-mono text-[11px] truncate max-w-[200px]" title={url}>
                          {url}
                        </span>
                        <span className="text-[10px] text-emerald-400 font-medium">✓ Crawled</span>
                      </div>
                    ))
                  ) : (
        ...
        {activeTab === 'role' && (
          <div className="space-y-6">
            <div className="pb-4 border-b border-[#212330]">
              <h2 className="text-lg font-bold text-white tracking-tight">Role Breakdown & Requirements</h2>
              <p className="text-xs text-slate-400 mt-0.5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              {/* Responsibilities */}
              <div className="lg:col-span-5 bg-[#10121B] border border-[#212330] rounded-xl p-6 shadow-sm">
                ...
                <div className="space-y-2.5">
                  {kit.role.requirements.map((req) => {
                    ...
                    return (
                      <div
                        key={req.id}
                        className="bg-[#10121B] border border-[#212330] rounded-xl p-4 flex items-start justify-between gap-4"
                      >
                        ...
                        <div>
                          {isUncovered ? (
                            <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 text-[10px] font-mono border border-amber-500/30">
                              <AlertTriangle className="w-3 h-3" />
                              <span>Gap</span>
                            </span>
                          ) : (
                            <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 text-[10px] font-mono border border-emerald-500/30">
        ...
        {activeTab === 'questions' && (
          <div className="space-y-6">
            {/* Action Bar */}
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-[#212330] gap-4">
              {/* Filter pills */}
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs font-medium capitalize transition-colors whitespace-nowrap ${
                      selectedCategory === cat
                        ...
                        : 'bg-[#12141E] text-slate-400 hover:text-slate-200 border border-[#212330]'
                    }`}
                  >
                    {cat.replace('-', ' ')}
                  </button>
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className="flex items-center space-x-1.5 px-3 py-1.5 rounded-md border border-[#262838] hover:bg-[#1C1E2B] text-slate-300 hover:text-white text-xs font-medium transition-colors"
                >
                ...
                {selectedCategory !== 'all' && (
                  <button
                    ...
                    className="flex items-center space-x-1.5 px-3 py-1.5 bg-[#171926] hover:bg-[#1E2133] border border-[#262838] text-indigo-400 hover:text-indigo-300 rounded-md text-xs font-medium transition-colors"
                  >
            ...
            {/* New Question Form */}
            {showAddQuestion && (
              <form onSubmit={handleAddQuestion} className="bg-[#10121B] border border-indigo-500/40 rounded-xl p-5 shadow-lg space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-mono uppercase tracking-wider text-indigo-400 font-semibold">
                    Add Custom Question
                  </h3>
                  <button type="button" onClick={() => setShowAddQuestion(false)} className="text-slate-500 hover:text-slate-300">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-[#0A0B10] border border-[#262838] rounded-md p-2.5 text-xs text-slate-100 outline-none focus:border-indigo-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Answer Outline / Notes</label>
                  <textarea
                    rows={3}
                    value={newQOutline}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[#0A0B10] border border-[#262838] rounded-md p-2 text-xs text-slate-200 outline-none"
                    >
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-300 mb-1">Target Requirement</label>
                    <select
                      value={newQReqIds[0] || ''}
                ...
                <div className="flex justify-end space-x-2 pt-2">
                  <button
                    ...
                    className="px-3 py-1.5 border border-[#262838] text-slate-400 rounded-md text-xs font-medium"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-md text-xs font-medium shadow-sm"
                  >
                    Add Question
                  </button>
                </div>
            ...
            <div className="space-y-4">
              {filteredQuestions.map((q, idx) => {
                ...
                return (
                  <div
                    key={q.id}
                    className={`bg-[#10121B] border rounded-xl p-5 transition-all shadow-sm ${
                      q.is_pinned
                        ? 'border-indigo-500/50 bg-[#121422]'
                        : q.is_edited
                        ? 'border-amber-500/30'
                        : 'border-[#212330]'
                    }`}
                  >
                    {/* Header */}
                    <div className="flex items-center justify-between pb-3 border-b border-[#1A1C29]">
                      <div className="flex items-center space-x-2">
                        {/* Reorder arrows */}
                        <div className="flex items-center space-x-0.5 text-slate-500 mr-1">
                          <button
                        ...
                        <span className="font-mono text-xs font-bold text-slate-300">{q.id.toUpperCase()}</span>

                        {/* Category Dropdown */}
                        <select
                          value={q.category}
                          onChange={(e) => handleChangeCategory(q.id, e.target.value as QuestionCategory)}
                          className="bg-[#1A1C28] text-slate-300 text-[11px] font-mono py-0.5 px-2 rounded border border-[#27293C] outline-none"
                        >
                          <option value="technical">technical</option>
                          <option value="behavioural">behavioural</option>
                          <option value="system-design">system-design</option>
                        ...
                        {/* Status Badges */}
                        {q.is_pinned && (
                          <span className="px-1.5 py-0.2 rounded text-[10px] font-mono bg-indigo-500/10 text-indigo-400 border border-indigo-500/30">
                            PINNED
                          </span>
                        )}
                        {q.is_edited && (
                          <span className="px-1.5 py-0.2 rounded text-[10px] font-mono bg-amber-500/10 text-amber-400 border border-amber-500/30">
                            EDITED
                          </span>
                        )}
                        {q.origin === 'user_added' && (
                      ...
                      <div className="flex items-center space-x-1.5">
                        <button
                          ...
                          className={`p-1.5 rounded hover:bg-[#1C1E2B] transition-colors ${
                            q.is_pinned ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'
                          }`}
                        ...
                        <button
                          ...
                          className="p-1.5 rounded hover:bg-[#1C1E2B] text-slate-400 hover:text-slate-200 transition-colors"
                          title="Edit Question"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 rounded hover:bg-[#1C1E2B] text-slate-500 hover:text-red-400 transition-colors"
                    ...
                    <div className="pt-3 space-y-3">
                      {isEditing ? (
                        <div className="space-y-3">
                          <div>
                            <label className="text-[11px] text-slate-400 font-mono">Prompt</label>
                            <textarea
                              rows={2}
                              value={editPrompt}
                              onChange={(e) => setEditPrompt(e.target.value)}
                              className="w-full bg-[#0A0B10] border border-[#262838] rounded-md p-2.5 text-xs text-slate-100 outline-none focus:border-indigo-500 mt-1"
                            />
                          </div>
                          <div>
                            <label className="text-[11px] text-slate-400 font-mono">Answer Outline</label>
                            <textarea
                              rows={3}
                              value={editOutline}
                        ...
                        <div>
                          ...
                          {/* Answer Outline */}
                          <div className="mt-3 p-3 rounded-lg bg-[#0A0B11] border border-[#1A1C28] text-xs text-slate-300 leading-relaxed">
                      ...
                      <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1">
                        <div className="flex items-center space-x-1.5">
                          ...
                          {q.requirement_ids.map(rId => (
                            <span
                              key={rId}
                              className="px-1.5 py-0.5 rounded bg-[#1B1D2C] text-slate-300 font-mono text-[10px] border border-[#26283C]"
                            >
                              {rId}
                            </span>
                          ))}
                          ...
                          {[1, 2, 3].map(dot => (
                            <span
                              ...
                              className={`w-2 h-2 rounded-full ${
                                dot <= q.difficulty ? 'bg-indigo-500' : 'bg-slate-700'
                              }`}
                            />
                          ))}
                        </div>
        ...
        {activeTab === 'flashcards' && (
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-[#212330] gap-4">
              <div>
                <h2 className="text-lg font-bold text-white tracking-tight">Practice Mode</h2>
                <p className="text-xs text-slate-400 mt-0.5">
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className="px-3 py-1.5 border border-[#262838] hover:bg-[#1C1E2B] text-indigo-400 hover:text-indigo-300 rounded-md text-xs font-medium transition-colors"
            ...
            {/* New Flashcard Form */}
            {showAddCard && (
              <form onSubmit={handleAddFlashcard} className="bg-[#10121B] border border-indigo-500/40 rounded-xl p-5 shadow-lg space-y-4">
                <h3 className="text-xs font-mono uppercase tracking-wider text-indigo-400 font-semibold">
                  Add Custom Flashcard
                </h3>
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Front (Prompt)</label>
                  <input
                    type="text"
                    required
                    value={newFront}
                    ...
                    className="w-full bg-[#0A0B10] border border-[#262838] rounded-md p-2.5 text-xs text-slate-100 outline-none"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Back (Explanation)</label>
                  <textarea
                    rows={3}
                    required
                    value={newBack}
              ...
              <div className="max-w-2xl mx-auto space-y-6">
                ...
                <div className="flex items-center justify-between text-xs text-slate-400">
                  ...
                  <span className="text-[11px] text-slate-500">
                    Press <kbd className="px-1 py-0.5 rounded bg-[#1B1D2A] text-slate-300 font-mono">Space</kbd> to reveal, <kbd className="px-1 py-0.5 rounded bg-[#1B1D2A] text-slate-300 font-mono">1-3</kbd> to rate
                  </span>
                </div>
                ...
                <div
                  ...
                  className="min-h-[260px] bg-[#10121B] hover:bg-[#121422] border border-[#212330] hover:border-indigo-500/40 rounded-2xl p-8 flex flex-col justify-between cursor-pointer transition-all shadow-xl text-center select-none"
                >
                  <div className="flex items-center justify-between text-[11px] text-slate-500 font-mono">
                    <span>{kit.flashcards[practiceIndex].id.toUpperCase()}</span>
                  ...
                  <div className="py-6 space-y-4">
                    ...
                    {isAnswerRevealed ? (
                      <div className="pt-4 border-t border-[#1C1E2B] animate-in fade-in duration-200">
                  ...
                  <div className="flex items-center justify-center space-x-1.5 text-[11px] text-slate-500">
                    ...
                    {kit.flashcards[practiceIndex].requirement_ids.map(rId => (
                      <span key={rId} className="font-mono text-slate-400">{rId}</span>
                    ))}
                  </div>
                </div>
                ...
                {isAnswerRevealed && (
                  <div className="bg-[#10121B] border border-[#212330] rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3 animate-in fade-in">
                    <span className="text-xs text-slate-300 font-medium">How confident do you feel?</span>
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => handleScoreCard(1)}
                        className="px-3.5 py-1.5 rounded-md bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 text-xs font-medium border border-rose-500/30 transition-colors"
                      >
                        1 &middot; Needs Review
                      </button>
                      <button
                        onClick={() => handleScoreCard(2)}
                        className="px-3.5 py-1.5 rounded-md bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 text-xs font-medium border border-amber-500/30 transition-colors"
                      >
                        2 &middot; Medium
                      </button>
                      <button
                        onClick={() => handleScoreCard(3)}
                        className="px-3.5 py-1.5 rounded-md bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-xs font-medium border border-emerald-500/30 transition-colors"
                      >
                        3 &middot; Confident
                      </button>
                    </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  <button
                    onClick={prevCard}
                    className="px-4 py-2 rounded-md border border-[#262838] hover:bg-[#1C1E2B] text-slate-300 text-xs font-medium"
                  >
                    &larr; Previous Card
                  </button>
                  <button
            ...
            <div className="space-y-4">
              {kit.schedule.days.map((day) => {
                ...
                return (
                  <div
                    key={day.day}
                    className={`bg-[#10121B] border rounded-xl p-5 transition-all ${
                      isCompleted ? 'border-emerald-500/40 opacity-75' : 'border-[#212330]'
                    }`}
                  >
                    <div className="flex items-center justify-between pb-3 border-b border-[#1A1C29]">
                      <div className="flex items-center space-x-3">
                        <button
                          ...
                          className={`w-5 h-5 rounded flex items-center justify-center border transition-colors ${
                            isCompleted
                              ...
                              : 'border-[#2D3042] bg-[#0A0B10] hover:border-slate-500'
                          }`}
                        >
                      ...
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-mono font-medium bg-[#1A1C29] text-indigo-400 border border-[#27293C]">
                        {day.minutes} min
                      </span>
                    </div>
                    ...
                    <div className="pt-3 space-y-2">
                      ...
                      <div className="space-y-1.5">
                        {dayQuestions.map(q => (
                          <div
                            key={q.id}
                            className="p-2.5 rounded-lg bg-[#0C0E15] border border-[#1E202E] text-xs flex items-start justify-between gap-3"
                          >
                            <div>
                              <span className="font-mono text-indigo-400 font-bold mr-2">{q.id.toUpperCase()}</span>
                              <span className="text-slate-200">{q.prompt}</span>
                            </div>
        ...
        {/* ========================================================================= */}
        {/* TAB 6: CREATIVE FEATURE - WEAK SPOTS & READINESS DIAGNOSTIC              */}
        {/* ========================================================================= */}
        {activeTab === 'readiness' && (
          <div className="space-y-6">
            <div className="pb-4 border-b border-[#212330]">
              <h2 className="text-lg font-bold text-white tracking-tight">Interview Readiness & Weak Spots</h2>
              <p className="text-xs text-slate-400 mt-0.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              <div className="bg-[#10121B] border border-[#212330] rounded-xl p-5 shadow-sm">
                ...
                <div className="w-full bg-[#1A1C29] h-1.5 rounded-full mt-3 overflow-hidden">
                  <div
              ...
              <div className="bg-[#10121B] border border-[#212330] rounded-xl p-5 shadow-sm">
                ...
                <p className="text-[11px] text-slate-400 mt-2">Cards scored in Practice Mode</p>
              </div>
              ...
              <div className="bg-[#10121B] border border-[#212330] rounded-xl p-5 shadow-sm">
                ...
                <p className="text-[11px] text-slate-400 mt-2">Requirements flagged with low confidence</p>
              </div>
            </div>

            {/* Requirement Breakdown */}
            <div className="bg-[#10121B] border border-[#212330] rounded-xl p-6 shadow-sm space-y-4">
              ...
              <div className="space-y-3">
                {readinessStats.requirementReadiness.map(({ req, status, scorePercent }) => (
                  <div
                    key={req.id}
                    className="p-4 rounded-lg bg-[#0C0E15] border border-[#1E202E] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
                  >
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span
                          className={`px-1.5 py-0.2 rounded text-[10px] font-mono uppercase ${
                            req.priority === 'must' ? 'text-rose-400 bg-rose-500/10' : 'text-sky-400 bg-sky-500/10'
                          }`}
                        >
                          {req.priority}
                        </span>
                        <span className="text-[10px] font-mono text-slate-500">{req.kind}</span>
                      </div>
                    ...
                    <div className="flex items-center space-x-3 shrink-0">
                      {status === 'ready' && (
                        <span className="px-2.5 py-1 rounded bg-emerald-500/10 text-emerald-400 text-xs font-mono font-medium border border-emerald-500/30">
                          Ready ({scorePercent}%)
                        </span>
                      )}
                      {status === 'moderate' && (
                        <span className="px-2.5 py-1 rounded bg-amber-500/10 text-amber-400 text-xs font-mono font-medium border border-amber-500/30">
                          Moderate ({scorePercent}%)
                        </span>
                      )}
                      {status === 'weak' && (
                        <span className="px-2.5 py-1 rounded bg-rose-500/10 text-rose-400 text-xs font-mono font-medium border border-rose-500/30">
                          Weak Spot
                        </span>
                      )}
                      {status === 'unpracticed' && (
                        <span className="px-2.5 py-1 rounded bg-[#1B1D2A] text-slate-400 text-xs font-mono border border-[#27293C]">
                          Not Practiced
                        </span>
                      )}
                    </div>
      ...
      {regenerationTarget && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#12141F] border border-[#2A2C3E] rounded-xl max-w-md w-full p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Your custom-written, edited, and pinned questions will be <strong className="text-emerald-400">strictly preserved</strong>. Only unmodified generated questions in this category will be replaced.
            </p>

            <div className="p-3 rounded-lg bg-[#0A0B10] border border-[#1F2130] text-[11px] text-slate-400 space-y-1">
              <div>✓ Preserves questions marked <span className="text-amber-400 font-mono">EDITED</span></div>
              <div>✓ Preserves questions marked <span className="text-indigo-400 font-mono">PINNED</span></div>
              <div>✓ Preserves questions marked <span className="text-emerald-400 font-mono">USER-ADDED</span></div>
            ...
            <div className="flex items-center justify-end space-x-2 pt-2">
              <button
                ...
                className="px-3.5 py-2 border border-[#262838] hover:bg-[#1C1E2B] text-slate-300 rounded-md text-xs font-medium"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-md text-xs font-semibold shadow-md shadow-indigo-600/20"
              >
