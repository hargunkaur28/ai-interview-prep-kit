import { AppendixAKit, Requirement, Question, Flashcard, ScheduleDay } from './types';
 ...
 */
export function validateAppendixAKit(kit: any): ValidationResult {
  const errors: string[] = [];
  ...
  if (!kit.source || typeof kit.source !== 'object') {
    ...
    if (typeof kit.source.jd_chars !== 'number') errors.push('source.jd_chars must be a number');
    if (typeof kit.source.researched_at !== 'string') errors.push('source.researched_at must be an ISO date string');
  ...
  if (!Array.isArray(kit.questions)) {
    ...
    kit.questions.forEach((q: any, index: number) => {
      ...
      if (!Array.isArray(q.requirement_ids)) {
        ...
        q.requirement_ids.forEach((reqId: string) => {
      ...
      if (typeof q.prompt !== 'string') errors.push(`Question "${q.id || index}" prompt must be a string`);
      if (typeof q.answer_outline !== 'string') errors.push(`Question "${q.id || index}" answer_outline must be a string`);
  ...
  if (!Array.isArray(kit.flashcards)) {
    ...
    kit.flashcards.forEach((f: any, index: number) => {
      ...
      if (typeof f.front !== 'string') errors.push(`Flashcard "${f.id || index}" front must be a string`);
      if (typeof f.back !== 'string') errors.push(`Flashcard "${f.id || index}" back must be a string`);
      if (Array.isArray(f.requirement_ids)) {
        f.requirement_ids.forEach((reqId: string) => {
          if (!requirementIds.has(reqId)) {
  ...
  if (!kit.schedule || typeof kit.schedule !== 'object') {
    ...
    if (!Array.isArray(kit.schedule.days)) {
      ...
      kit.schedule.days.forEach((d: any, index: number) => {
        if (d.day !== index + 1) {
          errors.push(`schedule.days[${index}] day number must be ${index + 1} (got ${d.day})`);
        }
        if (typeof d.focus !== 'string') errors.push(`schedule.days[${index}].focus must be a string`);
